...
class MessageValidator:
    ...
    def __init__(self, max_message_length: int = 240, max_payload_length: int = 1000):
        self.logger = logging.getLogger(__name__)
        self.max_message_length = max_message_length
        self.max_payload_length = max_payload_length

        # Patterns for validation
        self.meshtastic_id_pattern = re.compile(r'^!?[0-9a-fA-F]{8}$|^\^all$|^\^broadcast$')
    ...
    def sanitize_string(self, text: str, max_length: Optional[int] = None) -> str:
        ...
        if len(sanitized) > max_len:
            ...
            self.logger.warning(f"String truncated to {max_len} characters")

        return sanitized

    def validate_meshtastic_message(self, message: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if len(text) > self.max_message_length:
            return False, f"Message too long: {len(text)} > {self.max_message_length}"
    ...
    def validate_external_message(self, message: Dict[str, Any]) -> Tuple[bool, Optional[str]]:
        ...
        if destination and not self.validate_meshtastic_id(destination):
            return False, f"Invalid destination_meshtastic_id format: {destination}"

        # Validate payload length
        if payload and isinstance(payload, str) and len(payload) > self.max_payload_length:
            return False, f"Payload too long: {len(payload)} > {self.max_payload_length}"

        return True, None

    def sanitize_meshtastic_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'destination' in sanitized:
            ...
            if not self.validate_meshtastic_id(dest):
                self.logger.warning(f"Invalid destination, using broadcast: {dest}")
                dest = '^all'
            sanitized['destination'] = dest

        # Sanitize text
        if 'text' in sanitized:
            sanitized['text'] = self.sanitize_string(sanitized['text'], self.max_message_length)

        # Ensure channel_index is valid
        if 'channel_index' in sanitized:
            try:
                sanitized['channel_index'] = max(0, min(7, int(sanitized['channel_index'])))
            except (ValueError, TypeError):
    ...
    def sanitize_external_message(self, message: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if 'payload' in sanitized and isinstance(sanitized['payload'], str):
            sanitized['payload'] = self.sanitize_string(sanitized['payload'], self.max_payload_length)
